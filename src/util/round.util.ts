import {Game, PlayerScore, Round} from "../@types/state";

export const calculatePlayerScore = (rounds: Round[], player: string, maxRoundIndex?: number) => {
    if (!maxRoundIndex && maxRoundIndex !== 0) {
        maxRoundIndex = rounds.length - 1;
    }
    let limitedRounds = maxRoundIndex < 0 ? [] : rounds.slice(0, maxRoundIndex + 1);
